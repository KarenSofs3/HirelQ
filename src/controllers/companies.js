...
export const getCompanies = async (req, res) => {
  try {
    ...
    if (plan && !allowedPlans.includes(plan)) {
      return res
        .status(400)
        .json({ message: "Plan inválido. Debe ser free, pro o enterprise" });
    }
  ...
  try {
    ...
    if (!nombre || !email_contacto) {
      return res
        .status(400)
        .json({
          message: "Los campos nombre y email_contacto son obligatorios",
        });
    }
    ...
    if (
      error.code === 11000 &&
      error.keyPattern &&
      error.keyPattern.email_contacto
    ) {
      return res
        .status(409)
        .json({ message: "El email de contacto ya está registrado" });
    }
  ...
  try {
    ...
    if (updateData.email_contacto) {
      const existingCompany = await Company.findOne({
        email_contacto: updateData.email_contacto,
        _id: { $ne: id },
      });
      if (existingCompany) {
        return res
          .status(400)
          .json({
            message: "El email de contacto ya está registrado en otra compañía",
          });
      }
    }
...
export const deleteCompany = async (req, res) => {
  try {
    ...
    if (!company) {
      return res
        .status(404)
        .json({ message: "Compañía no encontrada o ya inactiva" });
    }
